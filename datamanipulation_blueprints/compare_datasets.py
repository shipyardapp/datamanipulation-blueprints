...
def get_args():
    ...
    parser.add_argument(
        '--source-file-name2',
        dest='source_file_name2',
        required=True)
    parser.add_argument(
        '--source-folder-name2',
        dest='source_folder_name2',
        default='',
        required=False)
...
def main():
    ...
    df1_only = merged_df.loc[lambda x: x['_merge']
                             == 'left_only'].drop(columns=['_merge'])
    df2_only = merged_df.loc[lambda x: x['_merge']
                             == 'right_only'].drop(columns=['_merge'])
    df_overlap = merged_df.loc[lambda x: x['_merge']
